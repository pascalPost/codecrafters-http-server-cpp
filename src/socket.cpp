...
namespace http_server {
    ...
    Socket::Socket(Socket &&other) noexcept : file_descriptor{std::exchange(other.file_descriptor, {})} {
    }

    Socket &Socket::operator()(Socket &&other) noexcept {
        std::swap(file_descriptor, other.file_descriptor);
        assert(file_descriptor.has_value());
        assert(!other.file_descriptor.has_value());
        return *this;
    }
    ...
    void Socket::set_options() const {
        ...
        const int reuse = 1;
        if (setsockopt(file_descriptor.value(), SOL_SOCKET, SO_REUSEADDR, &reuse,
                       sizeof(reuse)) < 0) {
    ...
    void Socket::bind(const unsigned port) const {
        ...
        if (::bind(file_descriptor.value(), reinterpret_cast<const sockaddr *>(&address),
                   sizeof(address)) != 0) {
    ...
    void Socket::connect(const std::string &hostname, const unsigned port) const {
        ...
        if (::connect(file_descriptor.value(), reinterpret_cast<const sockaddr *>(&address), sizeof(address)) < 0)
    ...
    void Socket::set_listen(const int connection_backlog) const {
        ...
        if (listen(file_descriptor.value(), connection_backlog) != 0) {
    ...
    Socket Socket::accept() const {
        ...
        const auto connected_fd = ::accept(
            file_descriptor.value(), reinterpret_cast<sockaddr *>(&client_addr),
    ...
    void Socket::write(const std::string &message) const {
        if (::write(file_descriptor.value(), message.c_str(), message.size()) < 0) {
    ...
    std::string Socket::read() const {
        ...
        const auto bytes_received = ::read(file_descriptor.value(), buffer.begin(), buffer.size() - 1);
