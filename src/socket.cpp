...
void Socket::bind(const unsigned port) const {
  ...
  address.sin_port = narrow_cast<in_port_t>(port);

  if (::bind(file_descriptor, reinterpret_cast<sockaddr *>(&address),
             sizeof(address)) != 0) {
...
Socket Socket::accept() const {
  ...
  const auto connected_fd =
      ::accept(file_descriptor, reinterpret_cast<sockaddr *>(&client_addr),
